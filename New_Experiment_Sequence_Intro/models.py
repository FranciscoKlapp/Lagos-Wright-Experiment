 ...
 #   def set_payoff_dm(self):
    ...
    #    seller = self.get_player_by_role("seller")
     ...
     #   if seller.offer_accept == True:
      #      seller.payoff1 = - buyer.output_request  #linear function
       #     seller.tokens_end = round(seller.tokens_begin+buyer.token_offer, 2)
        #    buyer.payoff1 = round((Constants.mu*buyer.output_request**Constants.eta)/Constants.eta, 2) #concave function
         #   buyer.tokens_end = round(buyer.tokens_begin - buyer.token_offer, 2)
    ...
    #    else:
     ...
     #   self.subsession.money_supply = Constants.token_endowment*self.session.num_participants
      #  self.subsession.lump_sum = round((1-Constants.gamma) * self.subsession.money_supply / self.session.num_participants,2)
       # buyer.interest = round(buyer.tokens_end*(1-Constants.gamma),2)
       # seller.interest = round(seller.tokens_end*(1-Constants.gamma),2)
   ...
   # def set_payoff_combine(self):
    ...
    #def set_payoff_final(self):
     #   for p in self.get_players():
      #      p.participant.vars['sequence1_payoff']= p.participant.payoff+Constants.point_endowment
...
#    investment = models.FloatField()
    ...
    instructions_DM = models.FloatField()
    quiz_output_DM = models.FloatField()
    quiz_output_DM_retry = models.FloatField()
    quiz_output_DM2 = models.FloatField()
    quiz_output_DM2_retry = models.FloatField()
    quiz_tokens_DM = models.FloatField()
    quiz_tokens_DM_retry = models.FloatField()
    quiz_tokens_DM2 = models.FloatField()
    quiz_tokens_DM2_retry = models.FloatField()

    instructions_CM = models.FloatField()
    quiz_cm_bid = models.FloatField()
    quiz_cm_bid_retry = models.FloatField()
    quiz_cm_output_offer = models.FloatField()
    quiz_cm_output_offer_retry = models.FloatField()
    quiz_prediction1 = models.FloatField()
    quiz_prediction2 = models.FloatField()
    quiz_prediction1_retry = models.FloatField()
    quiz_prediction2_retry = models.FloatField()

    quiz_choose_1 = models.StringField(
        label='',
        choices=[['Buyer', 'Consumer (earn Points, spend Tokens)'], ['Seller', 'Producer (earn Tokens, spend Points)']],
        doc="""This player's decision""",
        widget=widgets.RadioSelect
    )
    quiz_choose_2 = models.StringField(
        label='',
        choices=[['Buyer', 'Consumer (earn Points, spend Tokens)'], ['Seller', 'Producer (earn Tokens, spend Points)']],
        doc="""This player's decision""",
        widget=widgets.RadioSelect
  ...
  #  prediction1 = models.FloatField()
   ...
   # token_offer = models.FloatField(label='How many Tokens would you like to offer for those units?', min=0)
